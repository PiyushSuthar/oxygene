...
var transferCmd = &cobra.Command{
	...
	RunE: func(*cobra.Command, []string) error {
		...
		// Select recipient
		recipient_alias, err := handler.Root().PromptString("recipient alias", 5, 15)
		if err != nil {
			return err
		}
		...
		var alias Alias
		db.Find(&alias, "alias = ?", recipient_alias)

		var recipient_address string
		if alias.Alias != "" {
			recipient_address = alias.Address
		}

		recipient, err := utils.ParseAddress(recipient_address)
